 ...
 * @private
 */
async function diagnoseSkippedIPv6(delivery, dnsResolve, filterAddress) {
    const unresolved = delivery.mx.filter(entry => entry.exchange && !net.isIP(entry.exchange) && !entry.A.length && !entry.AAAA.length);
...
async function resolveIP(delivery) {
    ...
    if (!addressFound) {
        ...
        if (!firstError && dnsOptions.ignoreIPv6) {
            await diagnoseSkippedIPv6(delivery, dnsResolve, filterAddress);
        }
