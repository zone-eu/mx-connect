...
async function resolveRecordType(dnsResolve, exchange, type) {
    try {
        // getDnsResolver already reduces any answer to an array with no falsy entries
        return await dnsResolve(exchange, type);
    } catch (err) {
...
async function resolveIP(delivery) {
    ...
    const filterAddress = (ip, exchange) => {
        ...
        if (ip && ip.error) {
            if (!firstError) {
                ...
                // The explanation goes on the message, and the response is derived from it, so
                // the two cannot disagree. Leaving message as the resolver's raw text meant a
                // caller logging err.message and a caller reporting err.response described the
                // same failure differently. resolve-mx.createDnsError does the same.
                err.message = `Unable to resolve the IP address for the specified host [${ip.exchange}] of the Mail Exchange (MX) server for the domain "${delivery.domain}". ${dnsErrors[err.code] || err.message}`;
                err.response = `DNS Error: ${err.message}`;
                err.category = 'dns';
                // Not conditional the way resolve-mx's is, because it need not be:
                // resolveRecordType turns ENODATA/ENOTFOUND into an empty list, so an error
                // that reaches here is always a real lookup failure rather than an absence of
                // records, which is exactly the case resolve-mx also treats as temporary.
                err.temporary = true;
                firstError = err;
            }
            return false;
    ...
    const resolutionPromises = delivery.mx.flatMap(entry => {
        ...
        if (net.isIP(entry.exchange)) {
            Object.assign(entry, tools.addressBuckets(entry.exchange));
            return [];
        }
    ...
    if (!addressFound) {
        ...
        // that were passed over. Nothing is pushed to the list unless ignoreIPv6 is set, so
        // the option needs no second mention here.
        if (!firstError) {
