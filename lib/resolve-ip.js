...
async function resolveIP(delivery) {
    ...
    const filterAddress = (ip, entry) => {
        ...
        if (invalid) {
            tools.recordBlockedAddress(delivery, entry.exchange, ip, invalid);
    ...
    for (const entry of delivery.mx) {
        entry.A = entry.A.filter(ip => filterAddress(ip, entry));
        entry.AAAA = entry.AAAA.filter(ip => filterAddress(ip, entry));
    }
