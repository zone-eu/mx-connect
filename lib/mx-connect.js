...
function createPolicyResolver(delivery) {
    ...
    return async (domain, type) => {
        ...
        // getDnsResolver answers with a real array whatever the resolver returned, which is
        // what makes the filtering below something this can rely on: an object supplying its
        // own filter method would otherwise pick which addresses mailauth connects to
...
function buildDeliveryObject(options) {
    ...
    return {
        ...
        // DNS resolution options. Left as the caller gave them: every flag tools reads
        // defaults to off by being absent, and naming a resolver here is what kept
        // getDnsResolver from ever reaching its native dns.promises branch - the default
        // delivery, which supplies no dnsOptions at all, took the promisified callback path
...
async function runPipeline(delivery) {
    ...
    // Resolve DANE TLSA records if enabled - the step owns that check and returns at once
    // when it is not, rather than having the same condition asked on both sides of the call
