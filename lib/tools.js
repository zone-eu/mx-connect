 ...
 *
 * Whatever the source, the answer comes back as an array of records. See normalizeRecords.
 *
...
function getDnsResolver(dnsOptions) {
    ...
    if (typeof options.resolveRecords === 'function') {
        ...
        return async (domain, type = 'A') => normalizeRecords(await options.resolveRecords(domain, type));
    }

    if (typeof options.resolve === 'function') {
        return async (domain, type = 'A') => {
    ...
    return async (domain, type = 'A') => normalizeRecords(await (type === 'A' ? dnsPromises.resolve4(domain) : dnsPromises.resolve(domain, type)));
}

/**
 * Reduces whatever a resolver answered with to a list of records.
 *
 * A custom resolver is user code, so its answer is shaped here rather than taken as given.
 * Normalizing once is what lets every call site read the result as a list: the alternative,
 * guarding at each site, is what left four unequal defenses behind - one call site accepted
 * any value, another only dropped falsy entries for MX lookups, and only the MTA-STS path
 * refused a non-array outright, so a plain object carrying its own filter method decided
 * which addresses survived on three paths out of four.
 *
 * This settles the shape, not the contents. A resolver is configuration the caller supplies,
 * not something an answering nameserver controls, so it is not a trust boundary: an array-like
 * Proxy would still satisfy Array.isArray. What every address is actually judged by is
 * checkAddress at the call sites.
 *
 * @param {*} list - Whatever the resolver returned
 * @returns {Array} The records, with anything falsy dropped
 * @private
 */
function normalizeRecords(list) {
    ...
    return list.filter(record => record);
}
...
/**
 * Sorts one IP address into the A/AAAA pair an MX entry carries.
 *
 * Three steps build an entry around a single address - the caller's mx option, an IP literal
 * target, and an exchange that is already an address - and each spelled the same ternary pair
 * out for itself. A non-address yields two empty arrays, which is what the callers already
 * did with one.
 *
 * @param {string} ip - The address to place
 * @returns {{A: Array<string>, AAAA: Array<string>}} The address in the array for its family
 */
function addressBuckets(ip) {
    return {
        A: net.isIPv4(ip) ? [ip] : [],
        AAAA: net.isIPv6(ip) ? [ip] : []
    };
}
