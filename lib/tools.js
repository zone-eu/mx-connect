...
function expandTargets(addr) {
    ...
    if (isIPv4Compatible(addr) || addr.match(NAT64_WELL_KNOWN_PREFIX) || addr.match(IPV4_TRANSLATED_PREFIX)) {
        return [{ addr: ipaddr.fromByteArray(addr.toByteArray().slice(12)), embedded: true }];
    }
    ...
    if (addr.match(SIX_TO_FOUR_PREFIX)) {
        ...
        return [
            { addr, embedded: false },
            { addr: ipaddr.fromByteArray(addr.toByteArray().slice(2, 6)), embedded: true }
        ];
    }
...
function createInvalidIpError(description, reason) {
    ...
    const error = new Error(`Unable to deliver email to ${description}${detail}`);
