 ...
 * @private
 */
async function callConnectHook(delivery, options) {
    if (typeof delivery.connectHook !== 'function') {
        return;
    }
    // The hook is a public callback-style API, so a manual promise wrapper is
    // needed here - this is the boundary between callback and async code
...
function connectWithTimeout(options, maxTime) {
    return new Promise((resolve, reject) => {
        ...
        // Persistent handler (not once): a second error event after settling
        // would otherwise find no listener and crash the process as an
 ...
 * @private
 */
async function tryConnect(delivery, mx) {
    ...
    try {
        await callConnectHook(delivery, options);
    } catch (err) {
    ...
    try {
        const socket = await connectWithTimeout(options, delivery.maxConnectTime);
...
/**
 * Tries MX hosts in order until one connects successfully.
 *
 * Each attempt either:
 * - Returns successfully with connected socket
 * - Throws a fatal error (hook failure) immediately
 * - Moves on to the next host on retryable error
 *
 ...
 * @private
 */
async function connectToFirstAvailable(delivery, mxHosts) {
    let firstError = null;

    for (const mx of mxHosts) {
        const result = await tryConnect(delivery, mx);
 ...
 */
async function getConnection(delivery) {
    ...
    const sortedHosts = unsortedHosts.sort((a, b) => {
        ...
        return dnsOptions.preferIPv6 ? (b.ipv6 ? 1 : 0) - (a.ipv6 ? 1 : 0) : 0;
    });
    ...
    return connectToFirstAvailable(delivery, mxHosts);
}
