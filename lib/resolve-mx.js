...
async function resolveMX(delivery) {
    ...
    const filterAddress = (ip, isTarget) => {
        ...
        if (invalid) {
            tools.recordBlockedAddress(delivery, delivery.decodedDomain, ip, invalid);
            ...
            if (!firstError) {
                ...
                firstError = new Error(`Unable to deliver email to ${source}${detail}`);
    ...
    if (aResult.list.length) {
        delivery.mx = aResult.list.map(entry => ({
            ...
            A: [entry].filter(ip => filterAddress(ip)),
            AAAA: []
        }));
    ...
    if (!dnsOptions.ignoreIPv6) {
        ...
        if (aaaaResult.list.length) {
            delivery.mx = aaaaResult.list.map(entry => ({
                ...
                mx: false,
                A: [],
                AAAA: [entry].filter(ip => filterAddress(ip))
            }));
            if (!addressFound && firstError) {
                throw firstError;
            }
