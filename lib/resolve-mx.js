...
async function resolveMX(delivery) {
     ...
     * @private
     */
    const filterAddress = (ip, isTarget) => {
        const invalid = tools.checkAddress(delivery, delivery.decodedDomain, ip);
        if (invalid) {
            if (!firstError) {
                // An IP literal target was handed to us rather than resolved for the
                // domain, so blaming an MX lookup would send the operator hunting through
                // DNS for a record that does not exist
                const source = isTarget
                    ? `the IP address [${ip}] given as the delivery target`
                    : `the IP address [${ip}] resolved for the Mail Exchange (MX) server of "${delivery.domain}"`;
                firstError = tools.createInvalidIpError(source, invalid);
            }
            return false;
        }
        addressFound = true;
    ...
    if (delivery.isIp) {
        if (!filterAddress(delivery.decodedDomain, true) && firstError) {
            throw firstError;
        }
    ...
    // Drop falsy entries that a broken custom resolver may return - reading
    // .priority from them below would otherwise throw. An all-falsy list is
    // treated like an empty answer and falls through to the A/AAAA fallback.
    const mxRecords = mxResult.list.filter(entry => entry);
    if (mxRecords.length) {
    ...
    if (aResult.list.length) {
        ...
        // leave a rejected address behind as an entry holding nothing, which the IP step then
        // resolves all over again and rejects a second time.
        delivery.mx = [
            {
                priority: 0,
                exchange: domain,
                mx: false, // Mark as implicit MX (not from MX record)
                A: aResult.list.filter(ip => filterAddress(ip)),
                AAAA: []
            }
        ];
    ...
    // Step 3: Fallback to AAAA records (IPv6-only hosts). This is asked for even under
    // ignoreIPv6, where the addresses are then refused by the filter. Skipping the lookup
    // saved nothing, since reaching here already means the domain published no MX and no A
    // record, and it left an IPv6-only domain indistinguishable from one with no mail
    // service at all: the delivery bounced as "no MX server found" for what is really a
    // local setting, rather than waiting on a retryable error naming the address.
        ...
        delivery.mx = [
            {
                ...
                mx: false,
                A: [],
                AAAA: aaaaResult.list.filter(ip => filterAddress(ip))
            }
        ];
        if (!addressFound && firstError) {
            throw firstError;
