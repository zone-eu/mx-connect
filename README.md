...
- **dnsOptions** is an object for IP address related options
    ...
    - **nat64Prefixes** (array of CIDR strings, defaults to `[]`) The NAT64 prefixes your network runs, for example `['2a01:4f8:c17:b8f::/96']`. The well-known prefix `64:ff9b::/96` is always recognised; a prefix from your own address space cannot be, since nothing in the address identifies it. Declaring it lets the IPv4 address such an address carries be validated like any other, so a synthesized record pointing at `127.0.0.1` is refused while one pointing at a public host still delivers. All prefix lengths RFC 6052 allows (32, 40, 48, 56, 64 and 96) are understood, and an unparseable entry is ignored
