...
- **dnsOptions** is an object for IP address related options
    - **ignoreIPv6** (boolean, defaults to `false`) If true then no AAAA records are looked up and an IPv6 address given as the target is rejected. Note that it does not filter IPv6 addresses you supply yourself through the **mx** option, those are used as given
