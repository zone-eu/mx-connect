...
- **dnsOptions** is an object for IP address related options
    ...
    Validation applies to every address the library connects to, including addresses you pass in yourself through the **mx** option and the host a domain's MTA-STS policy is fetched from. Addresses dropped by these rules are listed on the `delivery.blockedAddresses` array handed to the `connectHook` and `connectError` callbacks, so filtering that still leaves a usable address can be told apart from a network failure.
